...
export function OverrideDecision({
    ...
    return (
      <div className="border-t border-rule-soft px-3 py-1.5">
        <button
          ...
          className="font-mono text-[11px] text-ink-3 underline decoration-dotted hover:text-accent"
        >
  ...
  return (
    ...
    >
      <label className="flex flex-wrap items-center gap-2 font-mono text-[12px] text-ink-3">
        accept instead
        <select
          value={accepted}
          onChange={(event) => setAccepted(event.target.value)}
          className="border border-rule bg-paper-2 px-1 py-0.5 font-mono text-[12px] text-ink"
        >
          {ACCEPTED.map((option) => (
      ...
      <label className="flex flex-col gap-1">
        ...
        <textarea
          ...
          maxLength={2000}
          required
          className="w-full max-w-prose border border-rule bg-paper-2 px-1.5 py-1 text-[13px] text-ink"
        />
      </label>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className={`border border-accent px-1.5 py-0.5 font-mono text-[11px] text-accent ${
