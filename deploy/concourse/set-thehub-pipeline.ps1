...
# -- B-59073 / B-59329: the pipeline source is `main`, and only `main` -------
#
# `fly set-pipeline` at the bottom of this script applies pipelines	hehub.yml,
# and it reads that configuration out of the `main` COMMIT rather than off
# disk. Why, what that closes, and why checking the tree faster would not have
# closed it, are all in PipelineSource.ps1 beside the code.
#
# That file is where this lives now. B-59073 hardened this script and left
# `set-pipeline.ps1` and `set-personal-soc-pipeline.ps1` with no guard at all;
# three copies kept in step is the same drift shape as one of three siblings
# being hardened, so there is one implementation and all three call it
# (B-59329).
...
# First of two evaluations. Here so an operator hears "no" in a second rather
# than after a minute of token-minting and a fly login; the one that actually
# governs what gets applied runs inside Resolve-PipelineConfig, on the line
...
try {
    ...
    # returns is the only `--config` there is. A script cannot skip the guard
    # and still have something to apply.
    $applied = Resolve-PipelineConfig -Root $PSScriptRoot -Pipeline $Pipeline `
        -RelativePath "pipelines\thehub.yml" -SourceRef $PipelineSourceRef `
