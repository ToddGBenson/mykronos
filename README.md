...
| 8-tab repo page (Dashboard, Findings, Harness, Threat Model, Supply chain, Insider Threat, Risk Decision, Remediation), portfolio/Findings count-mismatch fix, `triage`/Found-By filters, per-finding remediation preview + on-demand PR, SBOM download (spec 18) | **Done** — Threat Model is capability-level, not CWE-level (no `Finding` carries a structured CWE); its narrative layer is honest plumbing, no LLM wired (D-061) |
