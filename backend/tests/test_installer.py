...
SHELL_PATH = ".github/workflows/mykronos-sast-shell.yml"
POWERSHELL_PATH = ".github/workflows/mykronos-sast-powershell.yml"
...
def _render_lane(
    ...
    """Render one extra analyser lane exactly as `plan()` would.

    `already_enabled` renders only a capability's primary template, which is
    the whole state a repository was in before extra lanes existed. A test of
    the extra lane has to put its file on the branch too, or "already
    installed" and "never installed" are indistinguishable again.
    """
    return installer.templates.render(
        template,
        repo_full_name=onboarding.github_repo_full_name,
        default_branch=onboarding.default_branch,
        ingestion_api_url=installer.ingestion_api_url,
        ...
        mykronos_package_spec=installer.package_spec,
        config=config,
        gate_depends_on=_gate_depends_on("sast", onboarding.enabled_capabilities or []),
    ).content


class TestExtraAnalyserLanes:
    """A second analyser is a second lane, not a second capability (B-051).

    CodeQL implements no shell language and no PowerShell, so `keel` recorded
    47 successful SAST runs while most of it went unread. The fix is to run
    ShellCheck *beside* CodeQL rather than instead of it, chosen in the
    capability's own config. The lane selection itself is tested in
    `test_extra_analyser_lanes.py`; these are the installer's half.
    """

    async def test_the_extra_lane_is_staged_beside_the_primary_one(
        self, installer: WorkflowInstaller, onboarding: RepoOnboarding
    ) -> None:
        plan = await installer.plan(
            onboarding, {"sast"}, configs={"sast": {"extra_analysers": ["shellcheck"]}}
        )

        assert sorted(c.path for c in plan.changes) == sorted([SAST_PATH, SHELL_PATH])

    async def test_without_it_only_the_primary_lane_is_staged(
        self, installer: WorkflowInstaller, onboarding: RepoOnboarding
    ) -> None:
        """A repository does not get a second analyser it did not ask for."""
        plan = await installer.plan(onboarding, {"sast"})

        assert [c.path for c in plan.changes] == [SAST_PATH]

    async def test_both_lanes_upload_the_same_capability(
        self, installer: WorkflowInstaller, onboarding: RepoOnboarding
    ) -> None:
        """Which is what makes this an analyser rather than a new thing to
        enable: the coverage cross-check sees `sast` reporting either way."""
        plan = await installer.plan(
            onboarding,
            {"sast"},
            configs={"sast": {"extra_analysers": ["psscriptanalyzer"]}},
        )
        lane = [r for r in plan.rendered if r.path == POWERSHELL_PATH][0]

        assert re.search(r"capability:\s*sast\b", lane.content)
        assert re.search(r"tool:\s*psscriptanalyzer\b", lane.content)

    async def test_adding_an_analyser_to_a_live_capability_is_not_a_noop(
        self, installer: WorkflowInstaller, onboarding: RepoOnboarding, github: FakeGitHubClient
    ) -> None:
        """The capability set does not move, so a set comparison would say
        "nothing to do" while the repository never gains the lane. The same
        bug `languages` had, one field over."""
        await already_enabled(installer, github, onboarding, "sast")
        ...
        assert not plan.is_noop
        assert plan.updated == ["sast"]
        assert [c.path for c in plan.changes] == [SHELL_PATH]

    async def test_an_unchanged_analyser_config_is_still_a_noop(
        self, installer: WorkflowInstaller, onboarding: RepoOnboarding, github: FakeGitHubClient
    ) -> None:
        """Re-saving must not push a byte-identical diff of two files instead
        of one."""
        config = {"sast": {"extra_analysers": ["shellcheck"]}}
        await already_enabled(installer, github, onboarding, "sast", config=config)
        repo = github.repos[onboarding.github_repo_full_name]
        lane = _render_lane(installer, onboarding, "sast-shell", config["sast"])
        repo.files[SHELL_PATH] = lane
        repo.branches[repo.default_branch][SHELL_PATH] = lane
    ...
    async def test_disabling_the_capability_removes_the_extra_lane_too(
        self, installer: WorkflowInstaller, onboarding: RepoOnboarding, github: FakeGitHubClient
    ) -> None:
        """A lane left behind keeps uploading a capability the repository no
        longer has, and every one of those uploads is refused at the door
        (B-062) — a red workflow nobody can explain."""
        config = {"sast": {"extra_analysers": ["shellcheck"]}}
        await already_enabled(installer, github, onboarding, "sast", config=config)
        repo = github.repos[onboarding.github_repo_full_name]
        lane = _render_lane(installer, onboarding, "sast-shell", config["sast"])
        ...
        plan = await installer.plan(onboarding, set(), configs=config)

        assert plan.removed == ["sast"]
        assert sorted(c.path for c in plan.changes) == sorted([SAST_PATH, SHELL_PATH])
        assert all(c.is_delete for c in plan.changes)
